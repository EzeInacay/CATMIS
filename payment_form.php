...
if ($account_id === 0) die('Invalid account.');
...
.card-header h2 { font-size: 22px; color: #0f2027; margin-bottom: 16px; }
.info-row { display: flex; justify-content: space-between; font-size: 13px; color: #64748b; margin-bottom: 6px; }
.info-row span:last-child { font-weight: 600; color: #0f2027; }
.balance-row span:last-child { color: #dc2626; font-size: 18px; }

.field { margin-bottom: 20px; }
.field label { display: block; font-size: 12px; font-weight: 600; letter-spacing: 0.5px; text-transform: uppercase; color: #64748b; margin-bottom: 6px; }
...
.field input:focus, .field select:focus {
    border-color: #0077b6; background: white; box-shadow: 0 0 0 3px rgba(0,119,182,0.10);
}

.btn-row { display: flex; gap: 12px; margin-top: 8px; }
.btn-submit { flex: 1; padding: 13px; background: #0077b6; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; transition: background 0.2s; }
.btn-submit:hover { background: #005f8e; }
.btn-cancel { padding: 13px 20px; background: #f1f5f9; color: #64748b; border: none; border-radius: 8px; font-size: 14px; cursor: pointer; text-decoration: none; display: flex; align-items: center; transition: background 0.2s; }
...
<div class="card">
    ...
    <form method="POST" action="add_payment.php">
        ...
        <div class="field">
            <label>Amount (₱)</label>
            <input type="number" name="amount" min="1" max="<?= $data['balance'] ?>" step="0.01" placeholder="0.00" required>
        </div>
        ...
        <div class="field">
            ...
            <input type="text" name="or_number" placeholder="e.g. OR-2026-0001" required>
        </div>
